...
(define (get-pattern-bindings expr pattern literals shadowed)
  (cond ((pair? pattern)
         (cond ((and (pair? (cdr pattern)) (eq? '... (reduce-ident (cadr pattern))))
                (if (not (null? (cddr pattern)))
                    (error "Ellipsis not at the end of a list"))
                (if (list? expr)
                    (let ((bindings-lists
                            (map (lambda (subexpr) (get-pattern-bindings subexpr (car pattern) literals shadowed)) expr)))
                    ...
                    #f))
               ((pair? expr)
                (let ((car-bindings (get-pattern-bindings (car expr) (car pattern) literals shadowed))
                      (cdr-bindings (get-pattern-bindings (cdr expr) (cdr pattern) literals shadowed)))
...
(define (validate-expr expr patterns literals env)
  (let ((shadowed (if (null? literals) '() (env-shadowed env '()))))
    (define (loop patterns)
      (if (null? patterns)
          (error "Invalid expression")
          (if (not (get-pattern-bindings expr (car patterns) literals shadowed))
              (loop (cdr patterns)))))
    (loop patterns)))
...
(define (apply-derived-form expr literals rules env call-env)
  (let ((shadowed (if (null? literals) '() (env-shadowed call-env env))))
    (define (loop rules)
      (if (null? rules)
          (error "Invalid expression")
          (let ((bindings (get-pattern-bindings expr (caar rules) literals shadowed)))
            (if bindings
                (apply-pattern-bindings bindings (cadar rules) 0 env #t)
                (loop (cdr rules))))))
    (loop rules)))

(define (apply-macro expr literals rules env call-env)
  (let ((shadowed (if (null? literals) '() (env-shadowed call-env env))))
    (define (loop rules)
      (if (null? rules)
